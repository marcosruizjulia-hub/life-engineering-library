...
Separate useful preparation from avoidance.
Define one real-world test.
Set a 48-hour experiment.
Define evidence to review.
Output format:
Diagnosis
Avoidance pattern
...
Process loop:
Identify the goal.
Map intrinsic motives versus status motives.
Detect fear-based ambition.
Test alignment with values.
Design a sustainable ambition plan.
Output format:
...
Process loop:
Define the offer or shortcut.
Name the immediate reward.
Identify hidden costs and affected stakeholders.
Test the decision against publicity, reversibility, and character erosion.
Define a cleaner alternative path.
Recommend accept, modify, delay, or refuse.
Output format:
...
Process loop:
Clarify the desire.
Separate genuine connection from projection or consumption.
Assess consent, boundaries, power imbalance, and pressure.
Create one respectful request, or recommend disengagement if boundaries are negative or unclear.
Build a plan for accepting rejection without escalation.
Output format:
...
Map who was affected and how.
Separate guilt, shame, fear, and responsibility.
Identify what can be repaired and what cannot be undone.
Define one accountability step and one repair step.
Output format:
Reality check
Harm assessment
...
Process loop:
Restate the user's justification charitably.
Identify the emotional incentive behind it.
Expose the rationalization or fallacy.
Present the strongest counterargument.
Offer a cleaner action aligned with values.
Output format:
